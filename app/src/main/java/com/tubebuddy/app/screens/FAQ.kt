...
private fun defaultFaqs(): List<FAQ> = listOf(
    FAQ(
    ...
    FAQ(
        ...
        "Use warm water with a push–pull plunger motion (never force). Let the warm water sit 5–10 minutes and try again. " +
                "Some caregivers use enzyme + bicarbonate mixtures when approved for their tube type—keep your provider’s recipe handy."
    ),
    FAQ(
...
@Composable
private fun FAQItem(item: FAQ) {
    var expanded by remember { mutableStateOf(false) }
    val rotation by animateFloatAsState(if (expanded) 180f else 0f, label = "chevron")

    ListItem(
        headlineContent = { Text(item.q) },
        supportingContent = {
            AnimatedVisibility(visible = expanded) { Text(item.a) }
        },
        trailingContent = {
            Icon(
        ...
        modifier = Modifier
            .clickable { expanded = !expanded }   // tap anywhere on the row
    )
}
